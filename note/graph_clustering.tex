...
An alternative to setting a trade-off between the two conflicting objectives is to compute the set of \emph{Pareto optimal} solutions. A solution of a bi-objective optimization problem is Pareto optimal if there is no other solution with a better quality with respect to both objectives. Obtaining the set of Pareto optimal solutions for other types of bi-objective constrained clustering has been studied before~\cite{GunsDVD16}. To obtain the Pareto optimal solution, we use a method based on the $\epsilon$-constraint algorithm~\cite{DBLP0015143}. We remove the size of the smallest cluster from the objective and bound it by a hard constraint. Then we iteratively solve this modified problem, each time using a bound that is one higher than the size of the smallest cluster in the previous solution (see algorithm~\ref{alg:pareto}).
