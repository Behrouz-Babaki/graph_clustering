...
Figure~\ref{fig:nodecut} depicts the idea of a node-cut. The following theorem shows that connectivity of a graph can be characterized by its node-cut sets. 
...
\begin{figure}
\centering
\includegraphics{node_cut}
\caption{$S$ is a $uv$-node cut set. Each path between $u$ and $v$ passes through $S$\label{fig:nodecut}}
\end{figure}
...
\For{$i \in \{1, \ldots, k\}$}
	\For{$u, v$ such that $(u, v) \notin E$}
		\State $S^* \gets \text{min-cut}(u, v, z^{*i})$
		\If{$\sum_{w \in S^*} z^{*i}_w < z^{*i}_u + z^{*i}_v - 1$}
			\State $\mathcal{C} \gets \mathcal{C} \cup \{\sum_{w \in S^*} z_w \geq z_u + z_v - 1\}$
		\EndIf
	\EndFor
\EndFor
