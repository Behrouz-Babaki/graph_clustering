...
\NDB{Mention normalization (from Tias: as part of the construction of $G_o$, not as part of the problem definition)}
...
Clustering problems often have an inherent symmetry which is due to the fact that the labels of clusters are arbitrary. This means that for each solution, there are $k!$ equivalent solutions that only differ by the cluster labels. We can strengthen our formulation by breaking these symmetries. \cite{SheraliD05a} suggests two measures \NDT{this read 'options' but they are not (mutually excluding) options?} to reduce these symmetries: 1) assign label $1$ to the cluster that contains node $1$. 2) assign labels to other clusters in the increasing order of their sizes. This translates to the following constraints:
%
\begin{align}
&x_{11} = 1 \\
&\sum_{i=1}^{|V|} x_{ic} \leq \sum_{i=1}^{|V|} x_{i(c+1)} \qquad c \in \{1, \ldots, k-1\}
\end{align}
...
\label{sec:bnc}
 ...
 In a \emph{branch and cut} algorithm, cutting planes are added throughout the branch and bound tree. \NDT{removed a sentence about LP relaxation, too early and I don't get it ; )} %Note that even if we add some or none of the cuts to the linear programming (LP) relaxation, it will still provide a valid bound. Hence it is possible to add the cuts only when an integer solution is obtained.
...
According to theorem~\ref{theorem:cutset} to ensure that cluster $c$ is connected, the following condition should hold for each $uv$-node cut $S$:
...
Given a solution $\mathbf{x}^*$ of the problem, let us denote the value of $x_{ic}$ variables in this solution by $x^*_{ic}$ and the vector of variables corresponding to cluster $c$ by $\mathbf{x}^*_c$.If $\mathbf{x}^*$ is an integer solution, a constraint in~\ref{eq:connectivity} is violated if and only if $\sum_{w \in S} x^*_{wc} = 0$. However for non-integer solutions there could be different degrees of violation. To find the most violated constraint in cluster $c$ we need to find the following set for each non-adjacent pair $(u, v)$:
\begin{equation}
...
\For{$c \in \{1, \ldots, k\}$}
	\State $\delta \gets 0$
	\State $\mathcal{C}_c \gets \emptyset$	
	\For{$u, v$ such that $(u, v) \notin E$ and $x_{uc}^* + x_{vc}^* > 1$}
		\State $S^* \gets \text{min-cut}(u, v, {\mathbf{x}}^{*}_c)$ 
		\State $\Delta \gets x_{uc}^* + x_{vc}^* - 1 - \sum_{w \in S^*} x_{wc}^*$
		\If{$\Delta > \delta$ }\Comment{maximum violation}
			\State $\mathcal{C}_c \gets \{\sum_{w \in S^*} x_{wc} \geq x_{uc} + x_{vc} - 1\}$
			\State $\delta \gets \Delta$			
		\EndIf
	\EndFor
	\State $\mathcal{C} \gets \mathcal{C} \cup \mathcal{C}_c$
\EndFor
...
\NDBi{The numbers in table~\ref{table:runtime} are not correct. Just a placeholder!}

\begin{table}
\centering
\captionsetup{font=small}
\captionof{table}{Average runtimes of overlapping and non-overlapping clustering by enumerating all simple paths (ENUM) and branch and cut (BnC). The runtime for timed-out experiments is counted as 1800.}
\label{table:runtime}

\begin{tabular}{|ll|rr|rr|}
\hline
		 &   &  \multicolumn{2}{|c|}{overlapping} & \multicolumn{2}{|c|}{non-overlapping} \\
         &   & \cline{1-4}		 
         &   &  ENUM &  BnC &  ENUM &  BnC \\
instance & k &             &           &             &           \\
\hline
200\_1000 & 2 &     467.092 &    12.453 &     467.092 &    12.453 \\
         & 3 &     469.513 &    17.006 &     469.513 &    17.006 \\
         & 4 &     477.935 &    37.679 &     477.935 &    37.679 \\
\hline         
200\_1250 & 2 &     195.791 &    41.790 &     195.791 &    41.790 \\
         & 3 &     908.929 &    59.797 &     908.929 &    59.797 \\
         & 4 &    1725.501 &   185.785 &    1725.501 &   185.785 \\
\hline         
250\_1000 & 2 &     940.922 &    70.214 &     940.922 &    70.214 \\
         & 3 &    1691.626 &   159.455 &    1691.626 &   159.455 \\
         & 4 &    1800.000 &   200.899 &    1800.000 &   200.899 \\
\hline         
250\_750 & 2 &       0.089 &     4.033 &       0.089 &     4.033 \\
         & 3 &       0.357 &     5.554 &       0.357 &     5.554 \\
         & 4 &       0.669 &     9.273 &       0.669 &     9.273 \\
\hline         
300\_750 & 2 &     741.336 &    11.458 &     741.336 &    11.458 \\
         & 3 &     741.639 &    18.966 &     741.639 &    18.966 \\
         & 4 &     742.743 &    41.489 &     742.743 &    41.489 \\
\hline         
350\_500 & 2 &     466.669 &     0.116 &     466.669 &     0.116 \\
         & 3 &     466.677 &     0.185 &     466.677 &     0.185 \\
         & 4 &     466.684 &     0.192 &     466.684 &     0.192 \\
\hline         
350\_750 & 2 &       0.499 &    31.416 &       0.499 &    31.416 \\
         & 3 &       1.955 &    76.103 &       1.955 &    76.103 \\
         & 4 &       8.231 &   566.554 &       8.231 &   566.554 \\
\hline         
450\_750 & 2 &     742.028 &   106.779 &     742.028 &   106.779 \\
         & 3 &     607.781 &   499.499 &     607.781 &   499.499 \\
         & 4 &     426.364 &  1222.089 &     426.364 &  1222.089 \\
\hline
\end{tabular}
\end{table}
